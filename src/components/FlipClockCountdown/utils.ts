...
export function calcTimeDelta(target: Date | number | string): FlipClockCountdownTimeDelta {
    ...
    return {
        total: timeLeft,
        weeks: Math.floor(timeLeft / (24 * 60 * 60) / 7),
        days: Math.floor(timeLeft / (24 * 60 * 60) % 7),
...
export function parseTimeDelta(timeDelta: FlipClockCountdownTimeDelta): FlipClockCountdownTimeDeltaFormatted {
    ...
    return {
        weeks: {
            current: pad(timeDelta.weeks),
            next: pad(nextTimeDelta.weeks),
        },
        days: {
...
export const createDate = (year: number, month: number, day: number, hour: number, minute: number) => new Date(year, month - 1, day, hour, minute);
